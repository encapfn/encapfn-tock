...
use crate::otcrypto_mac_ef_bindings::{self, LibOTCryptoMAC};

const SHA_256_OUTPUT_LEN_BYTES: usize = 32;

pub struct OTCryptoLibHMAC<'a> {
    hmac_context: RefCell<otcrypto_mac_ef_bindings::hmac_context_t>,
...
impl OTCryptoLibHMAC<'_> {
    pub fn new() -> Self {
        OTCryptoLibHMAC {
            hmac_context: RefCell::new(otcrypto_mac_ef_bindings::hmac_context_t {
                inner: otcrypto_mac_ef_bindings::hash_context_t {
                    mode: 0,
                    data: [0; 52],
                },
                outer: otcrypto_mac_ef_bindings::hash_context_t {
                    mode: 0,
                    data: [0; 52],
                },
            }),
    ...
    where
        F: FnOnce(&mut otcrypto_mac_ef_bindings::hmac_context_t) -> R,
    {
    ...
    fn add_data_int(&self, data: &[u8]) -> Result<(), ErrorCode> {
        let res = self.with_hmac_context(|hmac_context| {
            ...
            unsafe {
                otcrypto_mac_ef_bindings::otcrypto_hmac_update(hmac_context as *mut _, msg_buf)
            };
        });
...
impl<'a> digest::DigestHash<'a, { SHA_256_OUTPUT_LEN_BYTES }> for OTCryptoLibHMAC<'a> {
    ...
    fn run(
        ...
        self.with_hmac_context(|hmac_context| {
            ...
            let mut tag_buf = otcrypto_mac_ef_bindings::crypto_word32_buf_t {
            ...
            unsafe impl encapfn::branding::EFID for UnsafeMockBranding {}
            let access = unsafe { encapfn::types::AccessScope::<UnsafeMockBranding>::new() };

            // Pretend that we're EF and use the validation infrastructure:
            let tag_array_ef_mut_ref = unsafe {
                core::mem::transmute::<
                    &mut [u32; 256 / 32],
                    encapfn::types::EFMutRef<UnsafeMockBranding, [u32; 256 / 32]>,
                >(&mut tag_array)
            };

            // Now, "validate" (will be a nop)
            let tag_array_ef_val = tag_array_ef_mut_ref.validate(&access).unwrap();
    ...
    fn set_mode_hmacsha256(&self, key: &[u8]) -> Result<(), ErrorCode> {
        ...
        self.with_hmac_context(|hmac_context| {
            let key_config_rust = otcrypto_mac_ef_bindings::crypto_key_config {
                ...
                security_level:
                    otcrypto_mac_ef_bindings::crypto_key_security_level_kSecurityLevelLow,
            };
            ...
            let keyblob_words =
                unsafe { otcrypto_mac_ef_bindings::keyblob_num_words(key_config_rust) };
            ...
            unsafe {
                otcrypto_mac_ef_bindings::keyblob_from_key_and_mask(
            ...
            let mut blinded_key = otcrypto_mac_ef_bindings::crypto_blinded_key_t {
            ...
            let checksum = unsafe {
                otcrypto_mac_ef_bindings::integrity_blinded_checksum(
