...
pub unsafe fn main() {
    ...
    encapfn::branding::new(|brand| {
        ...
        // Run a test:
        encapfn_tock_littlefs::test_littlefs(
            if cfg!(feature = "og_eval_disable_checks") { "ef_mock_unchecked" } else { "ef_mock_checked" },
    ...
    encapfn::branding::new(|brand| {
        ...
        // Run a test:
        encapfn_tock_littlefs::test_littlefs(
            if cfg!(feature = "og_eval_disable_checks") { "ef_mpk_unchecked" } else { "ef_mpk_checked" },
            &bound_rt, &mut alloc, &mut access, hardware_alarm);
    });
